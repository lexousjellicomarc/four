...
function statusPill(status?: string | null) {
  ...
  return cn(
    ...
    map[value] ?? 'bg-zinc-200 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-100',
  );
}
...
function stepsForBooking(booking: BookingPayload) {
  ...
  const hasConfirmedPayment = payments.some((payment) => String(payment.status ?? '').toLowerCase() === 'confirmed');
  ...
  return [
    {
      ...
      description: booking.created_at ? `Recorded on ${formatDateTime(booking.created_at)}` : 'Booking record has been created.',
      complete: true,
      icon: ClipboardList,
    },
    {
    ...
    },
    {
      ...
      description: hasConfirmedPayment || paymentStatus === 'paid'
...
function lifecycleTone(event: BookingLifecycleEvent) {
  ...
  if (toStatus === 'confirmed' || toStatus === 'active' || toStatus === 'completed') {
...
function lifecycleIcon(event: BookingLifecycleEvent) {
  ...
  if (toStatus === 'declined' || toStatus === 'cancelled' || toStatus === 'deleted') return Flag;
  return CalendarDays;
}
...
export default function BookingProgressPanel({ booking, compact = false }: Props) {
  ...
  const outstanding = Math.max(
    Number(booking.totals?.remaining_balance ?? itemsTotal - confirmedTotal),
    0,
  );
  ...
  return (
    <div className="space-y-6">
              ...
              <div key={step.key} className="flex gap-3">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className={statusPill(step.complete ? 'confirmed' : 'pending')}>
      ...
      <Card>
        ...
        <CardContent className={cn('grid gap-3', compact ? 'grid-cols-1' : 'grid-cols-2')}>
          <div className="rounded-2xl border p-4">
            <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Items total</div>
            <div className="mt-2 text-2xl font-semibold">₱ {formatMoney(itemsTotal)}</div>
          </div>

          <div className="rounded-2xl border p-4">
            <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Submitted payments</div>
            <div className="mt-2 text-2xl font-semibold">₱ {formatMoney(submittedTotal)}</div>
          </div>

          <div className="rounded-2xl border p-4">
            <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Confirmed payments</div>
            <div className="mt-2 text-2xl font-semibold">₱ {formatMoney(confirmedTotal)}</div>
          </div>
          ...
          {latest ? (
            <div className="rounded-2xl border p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={statusPill(latest.status)}>{latest.status ?? 'submitted'}</span>
              </div>
          ...
          {auditEntries.length > 0 ? (
            auditEntries.map((entry) => {
              ...
              return (
                <div
                  key={entry.key}
                  className={cn('rounded-2xl border p-4', entry.tone)}
                >
        ...
        <CardContent>
          <div className="rounded-2xl border p-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="font-medium">
                {booking.survey_proof_image_url ? 'Proof available' : 'Proof missing'}
              </div>
            </div>
            <div className="mt-2 text-sm text-muted-foreground">
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border p-4">
            ...
            <div className="mt-2">
              <span className={statusPill(booking.booking_status)}>{booking.booking_status ?? '—'}</span>
            </div>
          </div>
          ...
          <div className="rounded-2xl border p-4">
            ...
            <div className="mt-2">
              <span className={statusPill(booking.payment_status)}>{booking.payment_status ?? '—'}</span>
            </div>
          </div>

          <div className="rounded-2xl border p-4">
            <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Event window</div>
          ...
          <div className="rounded-2xl border p-4">
            ...
            <div className="mt-2 flex items-center gap-2 text-sm">
              ...
              <span>{booking.client_name ?? booking.company_name ?? '—'}</span>
            </div>
          </div>
        </CardContent>
      </Card>
