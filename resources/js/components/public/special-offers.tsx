...
const FAQ_ITEMS = [
  {
    ...
    answer:
      'Use the quick availability checker on the home page or the public calendar. Select the date, venue, event type, and guest count, then review the AM, PM, and EVE results before proceeding to booking.',
  },
  {
    question: 'Can I request several dates at once?',
    answer:
      'Yes. The schedule flow supports a main date range and, when allowed by staff controls, additional schedule entries for the same booking request.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'The request goes through schedule validation, client details review, and payment review. A booking only moves forward once the required payment stage is confirmed according to the current workflow.',
  },
  {
...
function answerFor(input: string) {
  ...
  if (text.includes('availability') || text.includes('available') || text.includes('date')) {
    return 'Choose the date and venue first, then review the AM, PM, and EVE block results. Blue means public event visibility, red means unavailable or blocked, and plain dates are still open under the current public view.';
  }
  ...
  if (text.includes('am') || text.includes('pm') || text.includes('eve') || text.includes('time block')) {
    return 'AM, PM, and EVE are separate schedule blocks. If you select two or more blocks, the whole-day logic should apply. Staff may also allow additional-hour items when the selected service type supports extension.';
  }

  if (text.includes('event') || text.includes('city') || text.includes('public')) {
    return 'Public events appear in blue on the client calendar so visitors can notice them and participate when applicable. Private bookings and blocked dates remain red or unavailable without exposing private details.';
  }
  ...
  if (text.includes('guideline') || text.includes('policy') || text.includes('terms')) {
    return 'The public guidelines page explains booking preparation, payment reminders, cancellation references, venue conduct, and client responsibilities. Use that page together with the contact form when you need clarification.';
  }
...
export default function SpecialOffers({ items = [] }: { items?: FeaturePackageItem[] }) {
  ...
  const chatBodyClass = messages.length > MAX_VISIBLE_MESSAGES ? 'max-h-[360px] overflow-y-auto pr-1' : '';
          ...
          <div className="mt-8 grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
            <div className="overflow-hidden rounded-[2rem] border border-black/5 bg-[#f8f4ea] dark:border-white/10 dark:bg-slate-900/70">
              <div className="relative h-[240px] overflow-hidden">
                <img src="/marketing/images/events/lightmain.JPG" alt="BCCC location" className="h-full w-full object-cover dark:hidden" />
                <img src="/marketing/images/events/darkmain.JPG" alt="BCCC location" className="hidden h-full w-full object-cover dark:block" />
              ...
              <div className="space-y-4 p-5 sm:p-6">
                <a href={openMapUrl} target="_blank" rel="noreferrer" className="flex items-start gap-3 rounded-[1.2rem] border border-black/5 bg-white px-4 py-4 text-sm text-slate-700 transition hover:-translate-y-0.5 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
                  <MapPin className="mt-0.5 h-4 w-4 shrink-0" />
                  <span>{siteSettings?.address || 'CH3X+RRW, Baguio, Benguet, Philippines'}</span>
                </a>

                <a href={`tel:${siteSettings?.phone || ''}`} className="flex items-center gap-3 rounded-[1.2rem] border border-black/5 bg-white px-4 py-4 text-sm text-slate-700 transition hover:-translate-y-0.5 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
                  <Phone className="h-4 w-4 shrink-0" />
                  <span>{siteSettings?.phone || '(074) 446 2009'}</span>
                </a>

                <a href={`mailto:${siteSettings?.email || ''}`} className="flex items-center gap-3 rounded-[1.2rem] border border-black/5 bg-white px-4 py-4 text-sm text-slate-700 transition hover:-translate-y-0.5 dark:border-white/10 dark:bg-white/5 dark:text-slate-200">
              ...
              {visibleOffers.length > 0 ? (
                <div className="grid gap-5 lg:grid-cols-2">
                  {visibleOffers.map((offer) => (
                    <article key={String(offer.id)} className="group overflow-hidden rounded-[2rem] border border-black/5 bg-slate-50 shadow-[0_18px_45px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-slate-950/60">
                      <div className="h-64 overflow-hidden">
                        <img src={offer.lightImage || offer.image} alt={offer.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-105 dark:hidden" />
                        <img src={offer.darkImage || offer.image} alt={offer.title} className="hidden h-full w-full object-cover transition duration-500 group-hover:scale-105 dark:block" />
                      </div>

                      <div className="space-y-3 px-5 py-5">
                        <div className="text-[11px] font-bold uppercase tracking-[0.24em] text-emerald-700 dark:text-emerald-300">Special Offer</div>
                        <h3 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">{offer.title}</h3>
                        ...
                        <a href={offer.href || '/contact'} className="inline-flex items-center rounded-full bg-[#174f40] px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90 dark:bg-[#2d47ff]">
                          {offer.buttonLabel}
                        </a>
                      </div>
                    </article>
        ...
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[2.2rem] border border-black/5 bg-white/86 p-5 shadow-[0_24px_70px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-white/5 sm:p-6 lg:p-7">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-600 dark:text-slate-300">
              This guide answers common client questions about dates, booking flow, payments, venues, public events, and policy reminders before they reach staff.
            </p>

            <div className="mt-6 rounded-[1.8rem] border border-black/5 bg-slate-50 p-4 dark:border-white/10 dark:bg-slate-950/60">
              <div ref={chatBodyRef} className={`space-y-3 ${chatBodyClass}`}>
                {messages.map((message, index) => (
                  <div
                    key={`${message.role}-${index}`}
                    className={`rounded-[1.2rem] px-4 py-3 text-sm leading-7 ${message.role === 'assistant' ? 'bg-white text-slate-700 dark:bg-white/5 dark:text-slate-200' : 'bg-[#174f40] text-white dark:bg-[#294CFF]'}`}
                  >
              ...
              <div className="mt-4 flex gap-3">
                <input
                  ...
                  placeholder="Ask about availability, payments, public events, venues, or guidelines..."
                ...
                />
                <button type="button" onClick={sendMessage} className="inline-flex min-h-[52px] items-center justify-center rounded-[1.2rem] bg-[#174f40] px-5 text-sm font-semibold text-white transition hover:opacity-90 dark:bg-[#294CFF]">
          ...
          <div className="rounded-[2.2rem] border border-black/5 bg-white/86 p-5 shadow-[0_24px_70px_rgba(15,23,42,0.08)] dark:border-white/10 dark:bg-white/5 sm:p-6 lg:p-7">
            ...
            <div className="mt-6 space-y-3">
              {FAQ_ITEMS.map((item, index) => {
                ...
                return (
                  <div key={item.question} className="overflow-hidden rounded-[1.4rem] border border-black/5 bg-slate-50 dark:border-white/10 dark:bg-slate-950/60">
                    <button type="button" onClick={() => setOpenFaq((prev) => (prev === index ? null : index))} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left">
