...
export function AppHeader({ breadcrumbs = [] }: AppHeaderProps) {
  ...
  return (
    <>
      <header className={`sticky top-0 z-40 border-b ${tone.headerClass}`}>
        <div className="flex h-16 items-center gap-3 px-4">
            ...
            <Sheet>
              ...
              >
                ...
                <SheetHeader className="border-b border-white/10 p-4 text-left">
                  <Link href={tone.homeHref} className={`rounded-2xl border p-3 ${tone.brandClass}`}>
                    <AppLogo />
                  </Link>
                  ...
                  <div className="mt-3">
                    ...
                    <p className="mt-1 text-lg font-black">{tone.label}</p>
                  </div>
                </SheetHeader>
                ...
                <div className="flex-1 overflow-auto p-3">
                  {mobileGroups.map((group) => (
                    <div key={group.title} className="mb-5">
                      <p className="mb-2 px-2 text-xs font-black uppercase tracking-[0.2em] opacity-55">
                        {group.title}
                      </p>
          ...
          <Link href={tone.homeHref} prefetch className="flex shrink-0 items-center">
            <AppLogo />
          </Link>
      ...
      {breadcrumbs.length > 1 ? (
        <div className={`hidden border-b px-4 py-3 md:block ${tone.headerClass}`}>
