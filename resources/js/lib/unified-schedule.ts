...
export const BLOCK_META: Record<BlockKey, { label: string; time: string; start: string; end: string }> = {
...
export function longDate(input: string | Date | null) {
  if (!input) return '';
  const date = input instanceof Date ? input : new Date(`${String(input).slice(0, 10)}T00:00:00`);
...
export function expandDateRange(start: string, end: string) {
  ...
  if (Number.isNaN(current.getTime()) || Number.isNaN(last.getTime())) return list;
...
export function eventStartsOnDate(event: ScheduleLikeEvent, targetDate: string) {
...
export function blockIntervalForDate(dateValue: string, block: BlockKey) {
  if (block === 'AM') return { start: `${dateValue}T06:00`, end: `${dateValue}T12:00` };
  if (block === 'PM') return { start: `${dateValue}T12:00`, end: `${dateValue}T18:00` };
  return { start: `${dateValue}T18:00`, end: `${shiftDateKey(dateValue, 1)}T00:00` };
}

export function eventTouchesBlockOnDate(event: ScheduleLikeEvent, dateValue: string, block: BlockKey) {
...
export function resolveBlockAvailable(blocks: any, key: BlockKey) {
  ...
  if (Array.isArray(blocks)) {
    const found = blocks.find((block) => (block?.key ?? block?.label) === key);
...
export function deriveDayStatus(options: {
  availability?: { day_status?: string | null; is_fully_booked?: boolean; AM?: boolean; PM?: boolean; EVE?: boolean } | null;
  ...
  const hasOwnBooking = isClient && events.some((event) => String(event.kind ?? '').toLowerCase() === 'booking');
  ...
  const unavailableCount = [availability?.AM, availability?.PM, availability?.EVE].filter((v) => v === false).length;
...
export function scheduleStatusLabel(status: ScheduleDayStatus) {
  switch (status) {
    ...
    case 'public_booked': return 'Public Event';
    case 'private_booked': return 'Reserved';
    case 'limited': return 'Partially Available';
    case 'full': return 'Fully Occupied';
    case 'my-booking': return 'My Booking';
...
export function scheduleStatusDescription(status: ScheduleDayStatus) {
  switch (status) {
    case 'blocked': return 'This date is blocked for internal schedule control.';
    case 'public_booked': return 'This date already has a public event or public schedule activity.';
    case 'private_booked': return 'This date already has a private booking or reserved activity.';
    case 'limited': return 'This date still has open time blocks, but some schedules are already occupied.';
    case 'full': return 'This date is fully occupied for the current schedule logic.';
    case 'my-booking': return 'You already have a booking on this date.';
    default: return 'This date is currently available.';
  }
}
