...
function formatRelativeDistance(target: Date | null, now: Date) {
  ...
  return diffMs >= 0 ? `Due in ${compact.trim()}` : `Overdue by ${compact.trim()}`;
}
...
export function getDeadlineSummary(booking: DeadlineBookingLike, now = new Date()): DeadlineSummary {
  ...
  const firstDeadline = createdAt ? new Date(createdAt.getTime() + 24 * 60 * 60 * 1000) : null;
  const finalDeadline = createdAt ? new Date(createdAt.getTime() + 48 * 60 * 60 * 1000) : null;
  ...
  const remainingToReachHalf = Math.max(downPaymentRequired - confirmedTotal, 0);
  ...
  const msToFinal = finalDeadline ? finalDeadline.getTime() - now.getTime() : null;
